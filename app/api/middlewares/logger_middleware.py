...
class LoggerMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        await self._process_response(
            request, response, response_body, start_time
        )
        return response

    @staticmethod
    async def _process_response(
        request, response: Response, response_body, start_time
    ):
        ...
        try:
            ...
            logger.info(
                "Response Body: [Could not decode body, might be binary data]"
            )
        logger.info(
            "<---- Response to request %s: %s",
            request.url,
    ...
    async def _process_request(request, body):
        ...
        if request.method in ("POST", "PUT", "PATCH"):
            try:
                ...
                logger.info(
                    "Request Body: [Could not decode body, might be binary data]"
                )
        logger.info(
