...
from app.data.schemas.solar.solar_schema import (
...
def backfill_missing_data():
    ...
    sites = SolarInstallation.select().where(
        (SolarInstallation.status == "Active")
        & (SolarInstallation.profile_updated_on.is_null(False))
        & (
            SiteRefYearProduction.select(
                fn.COUNT(SiteRefYearProduction.id)
...
def generate_2001_timestamps():
    ...
    return [
        datetime(2001, 1, 1) + timedelta(minutes=15 * i) for i in range(35040)
    ]
