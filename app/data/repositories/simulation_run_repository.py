...
from app.data.interfaces.i_simulation_run_repository import (
...
class SimulationRunRepository(
    BaseRepository[SimulationRuns], ISimulationRunRepository[SimulationRuns]
):
    def __init__(self):
    ...
    def count_simulation_run_by_status(self, simulation_container_id):
        query = (
            self._model.select(
                self._model.status, fn.COUNT(self._model.id).alias("count")
            )
            .where(
                self._model.simulation_container_id == simulation_container_id
            )
