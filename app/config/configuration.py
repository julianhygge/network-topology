...
class ApiConfiguration(metaclass=Singleton):
    ...
    def _get_otp_config(self):
        ...
        otp_config['max_otp_verification_attempts'] = int(otp_config.max_otp_verification_attempts)
        otp_config['max_resend_otp_attempts'] = int(otp_config.max_resend_otp_attempts)
        otp_config['max_resend_otp_attempt_window_in_min'] = int(otp_config.max_resend_otp_attempt_window_in_min)
    ...
    def _get_simulation_config(self):
        ...
        data['solar_peak_percentage_for_charging'] = float(data["solar_peak_percentage_for_charging"])
        data['load_peak_percentage_for_discharging'] = float(data["load_peak_percentage_for_discharging"])
        data['charge_battery_efficiency'] = float(data["charge_battery_efficiency"])
        data['discharge_battery_efficiency'] = float(data["discharge_battery_efficiency"])
        data['default_panel_capacity'] = float(data["default_panel_capacity"])
        data['default_battery_capacity_required'] = float(data["default_battery_capacity_required"])
        data['battery_dynamic_allocation'] = float(data["battery_dynamic_percentage_allocation"])
        data['battery_base_allocation'] = float(data["battery_base_percentage_allocation"])
        return data
