...
class AuthService(IAuthService):
    ...
    def request_otp(self, user, country_code):
        ...
        if len(attempts) >= self._max_resend_otp_attempts:
            ...
            self._auth_attempt_repository.update(
                auth_attempt.txn_id, **data_to_update_dict
            )

            return {
                "state_token": str(auth_attempt.txn_id),
                "attempts_remaining": self._max_otp_verification_attempts
        ...
        if account is None:
            with self._user_repository.database_instance.atomic():
                ...
                self._user_group_rel_repository.add_user_to_group(
                    account.id, account.id, Groups.Guest.value
                )
    ...
    def _new_attempt(self, user, country_code, otp, txn_id):
        ...
        data_response = {
            ...
            "attempts_remaining": self._max_otp_verification_attempts
            - new_attempt.verification_attempt_count,
        }
    ...
    def verify_otp(self, req_body, txn_id):
        ...
        session_user = self._user_repository.fetch_account_by_phone_number(
            phone_number=auth_attempt.phone_number
        )
        if not session_user:
